...
const RequestedAssets = () => {
  ...
  const handleAccept = async (id) => {
    try {
      const result = await axiosEmployee.put(`/assets/request/${id}`, {
        owner: verifiedUser?.email,
      });
      if (result?.status === 200) {
        Swal.fire({
          title: "Requested!",
  ...
  const handleReject = async (id) => {
    try {
      const result = await axiosEmployee.put(`/assets/cancel/${id}`, {
        owner: verifiedUser?.email,
      });
      if (result?.status === 200) {
        Swal.fire({
  ...
  return (
    <>
      <div>
        <Helmet>
          <title>Asset-Each | RequestedAsset Page</title>
        </Helmet>
                    ...
                    {request?.requestStatus !== "pending" ? (
                      <td>
                        <button
                          ...
                          className="btn bg-green-300 py-1"
                          disabled={
                            request?.availability === "Available" ? false : true
                          }
                        >
                          Request
                        </button>
